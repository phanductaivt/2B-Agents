...
_PATHS = _load_module(
    "project_paths_for_dependency_manager",
...
def _parse_registry(project_dir: Path) -> dict[str, object]:
    path = _PATHS.id_registry_path(project_dir)
    if not path.exists():
        return {}
...
def _parse_status_table(project_dir: Path) -> dict[str, dict[str, str]]:
    status_text = _read_if_exists(_PATHS.status_path(project_dir))
...
def _build_dependency_risks(project_dir: Path) -> list[str]:
    ...
    for file_path in requirement_files:
        ...
        if current_status == "Blocked":
            risks.append(
                f"{req_name} is blocked in _ops/status.md. Dependent UXUI/FE work should pause until BA gaps are resolved."
            )
...
def write_dependency_map(project_dir: Path) -> Path:
    path = _PATHS.dependency_map_path(project_dir)

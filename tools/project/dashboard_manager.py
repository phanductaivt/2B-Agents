        ...
        detailed_sections.extend(
            [
                ...
                *[f"  - {action}" for action in action_lines],
                "- Quick Links:",
                f"  - [Project README](./{project_dir.name}/README.md)",
                f"  - [Status](./{project_dir.name}/status.md)",
                f"  - [Decision Log](./{project_dir.name}/decision-log.md)",
                f"  - [Task Tracker](./{project_dir.name}/task-tracker.md)",
                f"  - [Outputs](./{project_dir.name}/outputs/)",
                "",
            ]
        )
    ...
    for project_dir in discover_projects(projects_dir):
        ...
        rows.append(
            "\n".join(
                [
                    ...
                    f"<td><a href=\"./{project_dir.name}/\">Open</a></td>",
                    "</tr>",
                ]
            )
        )
        ...
        card = "\n".join(
            [
                ...
                f"<a href=\"./{project_dir.name}/\">Project Folder</a>",
                f"<a href=\"./{project_dir.name}/status.md\">Status</a>",
                f"<a href=\"./{project_dir.name}/outputs/\">Outputs</a>",
                "</div>",
                "</div>",
            ]
        )
            ...
            blocked_cards.append(
                "\n".join(
                    [
                        ...
                        f"<p><a href=\"./{project_dir.name}/\">Open project</a></p>",
                        "</div>",
                    ]
                )
            )
...
def write_dashboard(projects_dir: Path) -> Path:
    dashboard_path = projects_dir / "dashboard.md"
    dashboard_path.write_text(build_dashboard(projects_dir), encoding="utf-8")
    return dashboard_path


def write_dashboard_html(projects_dir: Path) -> Path:
    dashboard_path = projects_dir / "dashboard.html"
...
def main() -> None:
    ...
    dashboard_path = write_dashboard(projects_dir)
    html_path = write_dashboard_html(projects_dir)
